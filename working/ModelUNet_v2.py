...
class ModelUNetV2(object): #maybe define prototype?
    @staticmethod
    ...
    def init_model(shape):
        ...
        model.compile(optimizer='adam', loss=iou_loss, metrics=[ModelUNetV2.mean_iou])
    ...
    def __init__(self, shape=(128,128,3), name="model_unet_v2"):
        self.model_area_file=name+"_area.h5"
        self.area_fit_history=None
        self.model_boundary_file=name+"_boundary.h5"
        self.boundary_fit_history=None

        if os.path.isfile(self.model_area_file):
            print("found model file")
            self.trained=True
            self.model=load_model(self.model_area_file, custom_objects={'mean_iou': ModelUNetV2.mean_iou,'iou_loss':iou_loss})
    ...
    def fit_model(self, train:Images):
        self.fit_model_area(train)
        self.fit_model_boundary( train)

    def fit_model_area(self, train:Images):
        self.model_area=ModelUNetV2.init_model(self.shape)
        earlystopper = EarlyStopping(patience=5, verbose=1)
        checkpointer = ModelCheckpoint(self.m_file, verbose=1, save_best_only=True)
        self.area_fit_history = self.model.fit(train.get_images(self.shape[:2]),
                    train.get_masks(self.shape[:2],labeled=False),
                    validation_split=0.1, batch_size=16, epochs=50,
                    callbacks=[earlystopper, checkpointer])
        self.trained_area=True

    def fit_model_boundary(self, train:Images):
        self.model_boundary=ModelUNetV2.init_model(self.shape)
        earlystopper = EarlyStopping(patience=5, verbose=1)
        checkpointer = ModelCheckpoint(self.m_file, verbose=1, save_best_only=True)
        self.boundary_fit_history = self.model.fit(train.get_images(self.shape[:2]),
                    train.get_mask_boundary(self.shape[:2]),
                    validation_split=0.1, batch_size=16, epochs=50,
                    callbacks=[earlystopper, checkpointer])
    ...
    def predict_unlabeld(self, img:Images, th=None):
        if self.trained:
            self.model = load_model(self.m_file, custom_objects={'mean_iou': ModelUNetV2.mean_iou,'iou_loss':iou_loss})
...
if __name__=='__main__':
    ...
    train.load_masks()
    model=ModelUNetV2(name='unet_v1_128x128.h5')
